...
namespace Magicka.GameLogic.Entities.Abilities.SpecialAbilities
{
	...
	{
		// Token: 0x06001A7E RID: 6782 RVA: 0x000B3E4C File Offset: 0x000B204C
		public Grenade(Animations iAnimation) : base(iAnimation, "#specab_grenade".GetHashCodeCustom())
		{
			if (Grenade.sModel == null)
			{
				lock (Game.Instance.GraphicsDevice)
				{
					Grenade.sModel = Game.Instance.Content.Load<Model>("Models/Missiles/liberty_grenade");
				}
			}
		}

		// Token: 0x06001A7F RID: 6783 RVA: 0x000B3EB4 File Offset: 0x000B20B4
		public override bool Execute(ISpellCaster iOwner, PlayState iPlayState)
		{
			...
			if ((state != NetworkState.Client && (!(iOwner is Avatar) || !((iOwner as Avatar).Player.Gamer is NetworkGamer))) || (state == NetworkState.Client && iOwner is Avatar && !((iOwner as Avatar).Player.Gamer is NetworkGamer)))
			{
				...
				Grenade.SpawnGrenade(ref missileInstance, iOwner, ref translation, ref direction);
				...
				if (NetworkManager.Instance.State != NetworkState.Offline)
				{
					...
					spawnMissileMessage.Type = SpawnMissileMessage.MissileType.Grenade;
		...
		// Token: 0x06001A80 RID: 6784 RVA: 0x000B3FF8 File Offset: 0x000B21F8
		internal static void SpawnGrenade(ref MissileEntity iMissile, ISpellCaster iOwner, ref Vector3 iPosition, ref Vector3 iVelocity)
		{
			...
			conditionCollection[0].Add(new EventStorage(new PlayEffectEvent(Grenade.EFFECT, false, true)));
			conditionCollection[0].Add(new EventStorage(new PlaySoundEvent(Banks.Additional, Grenade.SOUND, false)));
			...
			conditionCollection[1].Add(new EventStorage(new PlayEffectEvent(Grenade.EFFECT, false, true)));
			conditionCollection[1].Add(new EventStorage(new PlaySoundEvent(Banks.Additional, Grenade.SOUND, false)));
			...
			conditionCollection[2].Add(new EventStorage(new PlayEffectEvent(Grenade.PULSE, true)));
			iMissile.Initialize(iOwner as Entity, Grenade.sModel.Meshes[0].BoundingSphere.Radius * 0.75f, ref iPosition, ref iVelocity, Grenade.sModel, conditionCollection, false);
