...
namespace Magicka.Levels
{
	...
	{
		// Token: 0x06002095 RID: 8341 RVA: 0x000E6F24 File Offset: 0x000E5124
		public Lava(LavaEffect iEffect, ContentReader iInput, LevelModel iLevel, AnimatedLevelPart iParent) : base(iParent)
		{
			...
			this.mRenderData = new Lava.RenderData[3];
			for (int l = 0; l < 3; l++)
			{
				Lava.RenderData renderData = new Lava.RenderData(this.mNumVertices);
		...
		// Token: 0x06002096 RID: 8342 RVA: 0x000E7410 File Offset: 0x000E5610
		...
		// Token: 0x06002097 RID: 8343 RVA: 0x000E7460 File Offset: 0x000E5660
		...
		// (get) Token: 0x06002098 RID: 8344 RVA: 0x000E7487 File Offset: 0x000E5687
		...
		// (get) Token: 0x06002099 RID: 8345 RVA: 0x000E748F File Offset: 0x000E568F
		...
		// Token: 0x0600209A RID: 8346 RVA: 0x000E7498 File Offset: 0x000E5698
		public override void Update(DataChannel iDataChannel, float iDeltaTime, Scene scene, ref Matrix iTransform, ref Matrix iInvTransform)
		{
			...
			Lava.RenderData renderData = this.mRenderData[(int)iDataChannel];
		...
		// Token: 0x0600209B RID: 8347 RVA: 0x000E75CC File Offset: 0x000E57CC
		...
		// Token: 0x0600209C RID: 8348 RVA: 0x000E77F8 File Offset: 0x000E59F8
		protected unsafe override void Freeze(ref Vector3 iOrigin, ref Vector3 iDirection, float iSpread, float iMagnitude)
		{
			...
			Vector3 vector;
			Vector3.Transform(ref iOrigin, ref this.mInvTransform, out vector);
			Vector3 vector2;
			Vector3.TransformNormal(ref iDirection, ref this.mInvTransform, out vector2);
			int[] array = DetectFunctor.IntStackAlloc();
			fixed (int* ptr = array)
			{
				int num = 0;
				...
				for (int i = -num2; i <= num2; i++)
				{
					...
					Vector3.Transform(ref iDirection, ref quaternion, out seg.Delta);
					...
					if (playState != null)
					{
						...
						for (int j = 0; j < shields.Count; j++)
						{
							...
							Vector3 vector3;
							Vector3 vector4;
							if (shields[j].Body.CollisionSkin.SegmentIntersect(out scaleFactor, out vector3, out vector4, seg))
							{
								Vector3.Multiply(ref seg.Delta, scaleFactor, out seg.Delta);
							}
						}
					}
					num += this.mIceCollisionMesh.GetVerticesIntersectingArc(ptr + num, 2048, ref iOrigin, ref seg.Delta, num3);
				}
				for (int k = 0; k < num; k++)
				{
					this.mWaterFreezeVertices[ptr[k]] += iMagnitude;
				}
			}
			DetectFunctor.FreeStackAlloc(array);
		}

		// Token: 0x0600209D RID: 8349 RVA: 0x000E7988 File Offset: 0x000E5B88
		...
		private LavaMaterial mMaterial;
		...
		private Lava.RenderData[] mRenderData;

		// Token: 0x0200041E RID: 1054
		protected class RenderData : IRenderableObject
		{
			// Token: 0x0600209E RID: 8350 RVA: 0x000E79CA File Offset: 0x000E5BCA
			public RenderData(int iVertexCount)
			{
				this.mFrozenVertices = new float[iVertexCount];
			}

			// Token: 0x0600209F RID: 8351 RVA: 0x000E79DE File Offset: 0x000E5BDE
			...
			// (get) Token: 0x060020A0 RID: 8352 RVA: 0x000E79ED File Offset: 0x000E5BED
			public int Effect
			{
				get
				{
					return LavaEffect.TYPEHASH;
				}
			}

			// Token: 0x170007FE RID: 2046
			// (get) Token: 0x060020A1 RID: 8353 RVA: 0x000E79F4 File Offset: 0x000E5BF4
			...
			// (get) Token: 0x060020A2 RID: 8354 RVA: 0x000E79F7 File Offset: 0x000E5BF7
			...
			// (get) Token: 0x060020A3 RID: 8355 RVA: 0x000E79FA File Offset: 0x000E5BFA
			...
			// (get) Token: 0x060020A4 RID: 8356 RVA: 0x000E79FD File Offset: 0x000E5BFD
			...
			// (get) Token: 0x060020A5 RID: 8357 RVA: 0x000E7A05 File Offset: 0x000E5C05
			...
			// (get) Token: 0x060020A6 RID: 8358 RVA: 0x000E7A0D File Offset: 0x000E5C0D
			...
			// (get) Token: 0x060020A7 RID: 8359 RVA: 0x000E7A15 File Offset: 0x000E5C15
			...
			// Token: 0x060020A8 RID: 8360 RVA: 0x000E7A1D File Offset: 0x000E5C1D
			...
			// (get) Token: 0x060020A9 RID: 8361 RVA: 0x000E7A20 File Offset: 0x000E5C20
			...
			// Token: 0x060020AA RID: 8362 RVA: 0x000E7A28 File Offset: 0x000E5C28
			public void Draw(Effect iEffect, BoundingFrustum iViewFrustum)
			{
				LavaEffect lavaEffect = iEffect as LavaEffect;
				this.mFrozenVertexBuffer.SetData<float>(this.mFrozenVertices);
				lavaEffect.GraphicsDevice.Vertices[1].SetSource(this.mFrozenVertexBuffer, 0, 4);
				lavaEffect.GraphicsDevice.RenderState.ReferenceStencil = 3;
				lavaEffect.World = this.World;
				this.mMaterial.AssignToEffect(lavaEffect);
				lavaEffect.Time = this.mTime;
				lavaEffect.CommitChanges();
				lavaEffect.GraphicsDevice.DrawIndexedPrimitives(Microsoft.Xna.Framework.Graphics.PrimitiveType.TriangleList, 0, 0, this.mNumVertices, 0, this.mPrimitiveCount);
				lavaEffect.GraphicsDevice.Vertices[1].SetSource(null, 0, 0);
			}

			// Token: 0x060020AB RID: 8363 RVA: 0x000E7ADC File Offset: 0x000E5CDC
			public void DrawShadow(Effect iEffect, BoundingFrustum iViewFrustum)
			{
				LavaEffect lavaEffect = iEffect as LavaEffect;
				lavaEffect.World = this.World;
				this.mMaterial.AssignOpacityToEffect(lavaEffect);
				lavaEffect.CommitChanges();
				lavaEffect.GraphicsDevice.DrawIndexedPrimitives(Microsoft.Xna.Framework.Graphics.PrimitiveType.TriangleList, 0, 0, this.mNumVertices, 0, this.mPrimitiveCount);
			}
