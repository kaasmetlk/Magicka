...
// Type: Magicka.GameLogic.Entities.Abilities.SpecialAbilities.TractorPush
...
internal class TractorPush : SpecialAbility
{
  ...
  private TractorPush()
    : base(Magicka.Animations.cast_area_ground, "#item_musician_force_push".GetHashCodeCustom())
  {
  ...
  public TractorPush(Magicka.Animations iAnimation)
    : base(iAnimation, "#item_musician_force_push".GetHashCodeCustom())
  {
    TractorPush.sAudioEmitter = new AudioEmitter();
    TractorPush.sAudioEmitter.Forward = Vector3.Forward;
    TractorPush.sAudioEmitter.Up = Vector3.Up;
  ...
  public override bool Execute(Vector3 iPosition, PlayState iPlayState)
  {
    ...
    for (int index = 0; index < entities.Count; ++index)
    {
      ...
      {
        float num2 = 1.5f * (float) Math.Pow(15.0 / (double) num1, 0.5);
    ...
    if (this.mSpellCaster == null)
    {
      ...
      iEmitter = TractorPush.sAudioEmitter;
    }
    else
      iEmitter = this.mSpellCaster.AudioEmitter;
    AudioManager.Instance.PlayCue<PushSpell.PushMagnitudeVolumAdjustAndOthers>(Banks.Spells, TractorPush.SOUND_EFFECT, iVariables, iEmitter);
    return true;
  }
