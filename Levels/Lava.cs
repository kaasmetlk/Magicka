...
public class Lava : Liquid
{
    ...
    ref Matrix iInvTransform)
  {
    ...
    if (iDataChannel == DataChannel.None)
      return;
    Lava.RenderData iObject = this.mRenderData[(int) iDataChannel];
    ...
    float iMagnitude)
  {
    if (!this.mFreezable)
      return;
    Vector3.Transform(ref iOrigin, ref this.mInvTransform, out Vector3 _);
    Vector3.TransformNormal(ref iDirection, ref this.mInvTransform, out Vector3 _);
    int[] alloced = DetectFunctor.IntStackAlloc();
    fixed (int* numPtr = alloced)
    {
      ...
      {
        Quaternion result;
        Quaternion.CreateFromYawPitchRoll((float) index1 * iSpread1, 0.0f, 0.0f, out result);
        Vector3.Transform(ref iDirection, ref result, out seg.Delta);
        ...
        num1 += this.mIceCollisionMesh.GetVerticesIntersectingArc(numPtr + num1, 2048 /*0x0800*/, ref iOrigin, ref seg.Delta, iSpread1);
      }
  ...
  protected class RenderData : IRenderableObject
  {
    ...
    public int Effect => LavaEffect.TYPEHASH;
    ...
    {
      LavaEffect iEffect1 = iEffect as LavaEffect;
