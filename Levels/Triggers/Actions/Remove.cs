...
// Type: Magicka.Levels.Triggers.Actions.Remove
...
public class Remove(Trigger iTrigger, GameScene iScene, XmlNode iNode) : Action(iTrigger, iScene)
{
  ...
  protected override void Execute()
  {
    if (this.mIsSpecific)
    {
      ...
      if (byId is Character character)
      {
        if (this.Force)
    ...
    else
    {
      ...
      for (int iIndex = 0; iIndex < triggerArea.PresentCharacters.Count; ++iIndex)
      {
        ...
        if (presentCharacter != null && (this.mTypeHash == Remove.ANYID || presentCharacter.Type == this.mTypeHash || (presentCharacter.GetOriginalFaction & this.mFactions) != Factions.NONE))
        {
          presentCharacter.Kill();
          presentCharacter.Terminate(true, false);
        }
      }
    }
  }
